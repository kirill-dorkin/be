    ...
    () =>
      isLoading
        ...
        : isNoResultsState
          ? ts("clear-search")
          : tc("submit"),
    [isLoading, isNoResultsState, ts, tc],
  );
          ...
          <ComboboxInput
            endAdornment={
              <Button
                ...
                size="icon"
                type={isLoading || isNoResultsState ? "button" : "submit"}
                variant="ghost"
                className={cn(
                  "text-foreground/70 hover:text-foreground cursor-pointer",
                  isLoading && "text-foreground/50 pointer-events-none",
                )}
            ...
            inputProps={{
              onChange: (event) =>
                setSearchState((state) => ({
                  ...state,
                  status:
                    event.target.value.length >= minLetters
                      ? "LOADING"
                      : "IDLE",
                  inputValue: event.target.value,
                  showOptions:
                    event.target.value.length > 0 || state.showOptions,
                })),

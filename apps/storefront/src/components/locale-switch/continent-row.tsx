...
const ContinentRowComponent = ({
  ...
  return (
    <section>
      <Label className="border-bottom dark:text-primary block border-b border-stone-200 pb-3 pt-4 text-lg font-semibold leading-6 text-slate-700 md:pb-4 md:pt-6">
        {name}
      </Label>
          ...
          return (
            <div key={market.id} className="flex">
              <Button
                variant="ghost"
                className={cn(
                  "text-muted-foreground flex h-auto w-full flex-col items-start rounded-2xl border border-stone-200/60 bg-white/95 p-4 text-left text-sm font-normal leading-5 shadow-[0_20px_60px_-48px_rgba(15,23,42,0.45)] transition-colors",
                  {
...
export const ContinentRow = memo(
  ContinentRowComponent,
  (prevProps, nextProps) => {
    return (
